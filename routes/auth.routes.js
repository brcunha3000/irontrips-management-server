...
router.post("/signup", (req, res, next) => {
    ...
    // Check if required informations are empty strings
    if (
        firstName === "" ||
        lastName === "" ||
        username === "" ||
        ...
        res.status(400).json({
            message: "Provide email, password, names and username",
        });
        return;
    }
    ...
    User.findOne({ email })
        .then((foundUser) => {
            ...
            if (passwordCorrect) {
                ...
                const { firstName, lastName, username, email, _id } = foundUser;
